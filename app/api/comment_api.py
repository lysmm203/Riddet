...
def add_comment(title, username):
    commentdb = CommentDB(g.mysql_db, g.mysql_cursor)
    ...
    comment = Comment(request.json['text'], post, user)
...
def update_post(comment_id, title, username):
    commentdb = CommentDB(g.mysql_db, g.mysql_cursor)
    postdb = PostDB(g.mysql_db, g.mysql_cursor)
    userdb = UserDB(g.mysql_db, g.mysql_cursor)
    ...
    comment = Comment(request.json['text'], title, user)
